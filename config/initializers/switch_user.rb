SwitchUser.setup do |config|
  ...
  config.available_users_identifiers = { :user => :_id }
  ...
  config.redirect_path = lambda { |request, params|
    ...
    if params[:mnt]
      request.session[:dashboard] = params[:rdb]
      return params[:dt]
    end
    
    request.session[:dashboard] = params[:db]
    request.session[:return_dashboard] = params[:rdb]
    request.session[:return_destination] = params[:rdt]
    destination = params[:dt]
